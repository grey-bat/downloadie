...
- [x] **`sequential_downloader.py` line 70 — bare `except: pass`**
...
- [x] **`sequential_downloader.py` line 79 — size check `< 100 * 1024 * 1024` is heuristic**
...
- [x] **Multiple versioned scripts: `turbo_downloader.py`, `turbo_final.py`, `turbo_sequential.py`, `sequential_turbo.py`, `simple_turbo.py`, `takeout_turbo.py`, `takeout_turbo_v1.py`**
...
- [x] **`sequential_downloader.py` lines 8-18 — all config at module level with hardcoded absolute paths**
...
- [x] Document how to extract Google cookies for aria2c auth
- [x] Add `.gitignore` for download artifacts (`*.aria2`, `*.log`, `*.zip`)

## Now
- [x] Add `requirements.txt` if missing
- [x] Document how to extract Google cookies for aria2c auth
- [x] Add `.gitignore` for download artifacts (`*.aria2`, `*.log`, `activity.json`)

## Next
- [x] Auto-trigger Immich ingest when download completes
